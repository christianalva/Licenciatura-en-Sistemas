...
// Procedimiento recursivo para imprimir los dígitos de un número
procedure ImprimirDigitosNumero(num: integer);
begin
  if num <> 0 then begin
    ImprimirDigitosNumero(num div 10);  // Llama recursivamente para los dígitos anteriores
    write(num mod 10, ' ');  // Imprime el dígito actual
  end;
end;

// Procedimiento para imprimir los dígitos de cada número en el vector
procedure ImprimirDigitos (v: vector; dimL: integer);
begin    
     {-- Completar --}    
     if dimL > 0 then begin
		ImprimirDigitos(v, dimL - 1);  // Llama recursivamente para los elementos anteriores
		writeln;
		ImprimirDigitosNumero(v[dimL]);  // Imprime los dígitos del número actual
		writeln;
	end; 
end; 
