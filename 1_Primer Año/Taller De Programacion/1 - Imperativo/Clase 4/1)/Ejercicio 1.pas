...
{ Retornar la cantidad de códigos que existen en el árbol que son menores que un valor que se recibe como parámetro }
  ...
  //=========================== D) =============================
	...
	begin
		// Caso base: Si el árbol está vacío (a = nil), no hay códigos menores que buscar,
		// por lo tanto, se retorna 0.
		if a = nil then
			CantidadDeCodigosMenores := 0
  
		// Si el código del nodo actual es menor que el código dado (cod),
		// entonces este nodo cuenta como 1, más los códigos menores que se encuentren
		// en ambos subárboles (izquierdo y derecho).
		...
		// Si el código del nodo actual es mayor o igual al código dado (cod),
		// no se cuenta este nodo y se continúa la búsqueda únicamente en el subárbol izquierdo,
		// ya que todos los nodos en el subárbol derecho serán mayores o iguales que el nodo actual.
		else
...
comprendidos entre los dos valores recibidos (sin incluir). }
  ...
  begin
    if(a = nil) then 
		...
		// Si el código del nodo actual es menor o igual que `codigo1`, continuamos la búsqueda en el subárbol derecho
		// porque los nodos del subárbol izquierdo no podrán estar en el rango.
