...
Begin
  ...
  then begin
         ...
         {Escribir el código correspondiente para hacer el corrimiento y disminuir la dimensión lógica}
          ...
          // Calcular la cantidad de elementos que se van a eliminar (los que están entre posInferior y posSuperior)
         salto := posSuperior - posInferior + 1;
         
         // Desplazar los elementos del vector para sobrescribir los elementos eliminados
...
begin
	...
	//módulo que retorne la información (ordenada por código de producto de menor a mayor) de cada código par de producto junto a la cantidad total de productos vendidos.
		writeln('Código: ', L^.dato.codigoP, ' - Cantidad: ', L^.dato.cantidad);
		L:=L^.sig;
	end;
end;
